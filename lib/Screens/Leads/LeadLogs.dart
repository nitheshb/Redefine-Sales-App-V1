...
class LeadLogsScreen extends StatefulWidget {
   const LeadLogsScreen({required this.leadDetails, super.key});
  final leadDetails;
  @override
  _LeadLogsScreenState createState() =>
      _LeadLogsScreenState();
}
...
class _LeadLogsScreenState extends State<LeadLogsScreen> {
  int _currentStep = 3;
  ...
  Widget build(BuildContext context) {
    return Expanded(
      ...
      stream: DbSupa.instance.streamLeadActivityLog('${widget.leadDetails.id}').asStream(),
  ...
  currentStep: _currentStep < buildSteps(stepsData).length ? _currentStep : 0,
  onStepTapped: (pos) {
    setState(() {
      _currentStep = pos;
    });
  ...
  steps: buildSteps(stepsData),
           ...
           return Text('No lead activity logs found. ${snapshot.data!.length}');
          return Expanded(
